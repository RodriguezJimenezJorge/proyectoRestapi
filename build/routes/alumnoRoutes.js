...
class AlumnoRoutes {
    constructor() {
        ...
        this.getDelete = (req, res) => __awaiter(this, void 0, void 0, function* () {
            const { nombre } = req.params;
            yield database_1.db.conectarBD();
            yield alumno_1.Alumnos.findOneAndDelete({ _nombre: nombre }, (err, doc) => {
                if (err)
                    console.log(err);
                else {
                    if (doc == null) {
        ...
        this.actualiza = (req, res) => __awaiter(this, void 0, void 0, function* () {
            const { nombre } = req.params;
            const { nota1, nota2, nota3, edad } = req.body;
            yield database_1.db.conectarBD();
            yield alumno_1.Alumnos.findOneAndUpdate({ _nombre: nombre }, {
                _nombre: nombre,
                _nota1: nota1,
                _nota2: nota2,
                _nota3: nota3,
    ...
    misRutas() {
        this._router.get('/', this.getAlumnos);
        this._router.get('/nuevoG/:nombre&:base&:altura&:lado1&:lado2', this.nuevoAlumnoGet);
        this._router.post('/nuevoP', this.nuevoAlumnoPost);
